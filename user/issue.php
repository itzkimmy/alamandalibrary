...
if (isset($_SESSION['login_user'])) {
    $sql = "SELECT user.username,uic,books.bid,name,authors,issue,issue_book.return, issue_book.approve FROM user
            INNER JOIN issue_book ON user.username=issue_book.username
            INNER JOIN books ON issue_book.bid=books.bid
            WHERE issue_book.username ='$_SESSION[login_user]' AND issue_book.approve !=''
